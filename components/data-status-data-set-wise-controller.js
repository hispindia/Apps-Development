...
DataStatusApp.controller('DataStatusDataSetWiseController',
    function ($rootScope, $scope, $http, $location, periodService, $window, organisationUnitGroupService, sqlviewservice,
            ...
            $.get(url, function (data) {
                for (var i = 0; i < data.dataSets.length; i++) {
                    if (data.dataSets[i].periodType === periodType) {
                        ...
                        if(check) $scope.dataSets.push($scope.dataSetss);
        ...
        $scope.buttonRight = function () { // selected id
            ...
            $scope.currentSelection.dataSet.forEach(child => {
                $scope.dataSetArray.push(child);
                $scope.dataSets = $scope.dataSets.filter( data => data.id != child.id);
            })

            $scope.dsId = $scope.dataSetArray.map(child => child.id); ; 
        ...
        $scope.currentSelection.dataSetArray.forEach(child => {
            $scope.dataSets.push(child);
            $scope.dataSetArray = $scope.dataSetArray.filter( data => data.id != child.id);
        })
