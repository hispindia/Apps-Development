...
export class GenerateReport extends Component {
  ...
  componentDidMount() {
    ApiService.getReportList()
      .then((response) => {
        response = response.reports.filter((report) =>
          report.name.includes("FHI360")
        );
        console.log(response);
        response = response.map((report) => ({
          ...report,
          name: report.name.replace("FHI360", ""),
        }));
  ...
  componentDidUpdate(prevProps) {
    if (prevProps.organisationUnit.id != this.props.organisationUnit.id)
      this.setState({ displayFrame: false });
  }
  ...
  displayYears = (totalYears) => {
    ...
    for (let i = year; i >= year - totalYears; i--)
      period += `<option value="${i}">${i}</option>`;
  ...
  handleClick(e, id) {
    ...
    if (id == "reportId") {
      this.state.reportParams[e.target.value].paramReportingPeriod
        ? this.setState({ periodSelection: true })
        : this.setState({ periodSelection: false });
    }
  }
  ...
  displayData = () => {
    ...
    if (this.state.periodSelection && !this.state.period)
      err.push("Please select a year!");
    err.length
      ? this.setState({ errors: err })
      : this.setState({ displayFrame: true, errors: [] });
  };
  ...
  render() {
    ...
    return (
      <Section>
        ...
        <CenterItem>
          <Button color="primary" onClick={this.displayData}>
            Generate Report
          </Button>
        </CenterItem>
