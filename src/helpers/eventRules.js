export const eventRules = (values, stage, rules // { rules, optionSets, pushChanges, updateValue }
  ...
  const findSection = id => {
    for (const section of stage) {
      if (section.id === id) return section; //       if (section.childSections) {
  ...
  const setColors = (condition, affected, testValue) => {
    if (!affected.optionSetValue) return; // if (affected.optionSet !== TEST_RESULT_ELEMENT) return

    const variables = getVariables(condition);
    variables.forEach(id => {
      const dataElement = stage[stage.dataElementIndex[id]].dataElements[id];

      if (values[id] && /\d/.test(dataElement.displayFormName) && variables.find(v => v !== dataElement.id && values[v] && values[v] !== '')) {
        dataElement.color = '';
        dataElement.warning = 'MIC is prioritized over DD';
      } else {
        dataElement.color = values[id] === '' || !testValue ? '' : getColor(testValue);
        if (dataElement.warning = 'MIC is prioritized over DD') dataElement.warning = '';
      }
    });
  };
  ...
  rules.forEach(rule => {
    rule.programRuleActions.forEach(r => {
      try {
        ...
        const de = r.dataElement ? stage[stage.dataElementIndex[r.dataElement.id]].dataElements[r.dataElement.id] : null;
        const s = r.programStageSection ? findSection(r.programStageSection.id) : null;

        switch (r.programRuleActionType) {
          case 'SHOWOPTIONGROUP':
            if (!cond || de.optionSet === r.optionGroup.id) break;
            de.optionSet = r.optionGroup.id; // Only reset selected value if the options do not include current value.
            // if (!optionSets[de.optionSet].find(option => option.value === values[de.id]) && values[de.id] !== '') {
          ...
          case 'SHOWWARNING':
            if (cond && de.warning !== r.content) de.warning = r.content;else if (!cond && de.warning === r.content) de.warning = null;
            break;

          case 'SHOWERROR':
            if (cond && de.error !== r.content) de.error = r.content;else if (!cond && de.error === r.content) de.error = null;
            break;
  ...
  }); // const invalid = validateValues(stage.dataElements, values, stage.programStageSections);
