        ...
        (acc, event, index) => {
          ...
          if (ubudeheCategory && patient.ubudehe !== ubudeheCategory) {
            acc.skipped[event.event] = this.skipEvent(
              event,
              1,
              `Patient is not part of the selected ubudehe category ${patient.ubudehe}`
            );
            return acc;
          }
          ...
          if (ageInDays / 30.25 < minAge || ageInDays / 30.25 > maxAge) {
            acc.skipped[event.event] = this.skipEvent(
              event,
              2,
              `Patient data is outside of specified age range. (${
                Math.round((ageInDays / 30.25) * 100) / 100
              } months)`
            );
            return acc;
          }
          ...
          // const rawAcfa = getMUACForAge(patient.gender, ageInDays);
