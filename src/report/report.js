    ...
    ["d2Map", "d2Meta", "d2Data", "d2Utils", "$q", "requestService",
        function(d2Map, d2Meta, d2Data, d2Utils, $q, requestService) {
            ...
            function vaccineReport() {
                ...
                //Period
                self.current.periods = monthsInYear(self.selectedPeriod);
                //Orgunit
            ...
            function vaccineReportProcessData() {
                ...
                for (let i = 0; i < dataColumns.length; i++){
                    if(i < 3){
                        newData.push(dataColumns[i]);
                    }else{
                        preData.push(dataColumns[i]);
                    }
                }
                var mergeData = [...preData, ...newData];
                self.current.headerColumns = headerColumns;
                self.current.dataColumns = mergeData;
            ...
            function performanceReportProcessData() {
                ...
                for( let i =0; i< columnsData.length; i++){
                    if(i<3){
                        newYear.push(columnsData[i]);
                    }else{
                        preYear.push(columnsData[i]);
                    }
                }
                let mergeYear = [...preYear, ...newYear];
                self.current.dataHeader = columns;
                self.current.dataHeaderData = mergeYear;
            ...
            function performanceTimeSummaryChart() {
                ...
                var months = ["Shrawan", "Bhadra", "Ashwin", "Kartik", "Mangsir", "Poush", "Magh", "Falgun", "Chaitra", "Baishak", "Jestha", "Asar"].slice(0, ++j);
            ...
            function monitoringChart(title, series) {
                ...
                self.monitoringChartData = {
                    ...
                    title: title,
                    categories: ["Shrawan", "Bhadra", "Ashwin", "Kartik", "Mangsir", "Poush", "Magh", "Falgun", "Chaitra", "Baishak", "Jestha", "Asar"],
                ...
                getRimCodes().then(function(codes) {
                    ...
                    $q.all(indicatorRequests).then(function(datas) {
                        ...
                        for (let currentMonth = 1; currentMonth <= selectedMonthId; ++currentMonth){
                            ...
                            const pe = self.selectedPeriod + zeroPaddedMonth;
            ...
            function timeSeries() {
                ...
                series.push({
                    "base": true,
                    "year": self.selectedPeriod,
                    "periods": monthsInYear(self.selectedPeriod)
                });
                if (self.selectedMonitoringReport.id === "allVac") return series;
                var currentYear = parseInt(self.selectedPeriod);
