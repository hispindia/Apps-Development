...
export const Panel = ({ showEdit }) => {
  ...
  const getDataElement = (id) => {
    ...
    switch (id) {
      ...
      //     })
      case "typeOfIsolate":
        let selectedIsolates = typeOfIsolates;

        if (
          PurposeOFSample &&
          PurposeOFSample["lJm7JZvPQxA"] === "Surveillance"
        ) {
          selectedIsolates = [typeOfIsolates[1]];
        }
  ...
  return (
    <CardSection heading="Panel" buttons={showEdit && <PanelButtons />}>
      <Grid container spacing={0}>
        ...
        <Grid item xs>
          {program && organisms.length
            ? program !== "L7bu48EI54J"
              ? getDataElement("organism")
              : ""
            : ""}
          {program && organisms.length
            ? program !== "L7bu48EI54J"
              ? getDataElement("typeOfIsolate")
              : ""
            : ""}
