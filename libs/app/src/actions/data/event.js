...
export const getAdmissionDate = (payload) => (dispatch) =>
  dispatch(createAction(CALCULATE_DAY, payload));
...
export const AddAndSubmit = (val) => (dispatch) =>
  dispatch(createAction(REMOVE_BUTTONS, val));
export const setButtonLoading = (payload) => (dispatch) =>
  dispatch(createAction(SET_BUTTONS, payload));
export const addExistingEvent = (payload) => (dispatch) =>
  dispatch(createAction(SET_EVENT, payload));
export const addSelectedProgramOfOrgUnits = (payload) => (dispatch) =>
  dispatch(createAction(SET_INITIAL_PROGRAM, payload));
    ...
    if (data.eventList.length == 0) {
      ...
      state.metadata.calculatedVariables.forEach((variables) => {
        for (let key in data.eventValues)
          if (key != variables.id && data.program == variables.program)
            data.eventValues[variables.id] = "";
      });
...
export const createNewEvent = () => async (dispatch, getState, followup) => {
  ...
  if (Object.keys(prevStateValues).length != 0) {
    Object.keys(prevStateValues).forEach(function(previouskey) {
      if (prevStateValues[previouskey] != "") {
        if (prevStateValues[previouskey] != "Organism growth detected") {
          values_to_send.push({
  ...
  try {
    ...
    metadata.calculatedVariables.forEach((variables) => {
      for (let key in data.eventValues)
        if (key != variables.id && panel.program == variables.program)
          data.eventValues[variables.id] = "";
    });
...
export const nextEvent = (next, addMoreSample, addMoreIso) => async (
  dispatch,
  getState
) => {
  const eventId = getState().data.event.id;
