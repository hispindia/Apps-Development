...
const Events = ({ match, history }) => {
  ...
  var Test = useSelector((state) => state.data);
  useEffect(() => {
    $("#msg1").hide();
    $("#succes1").hide();
  });
  ...
  return (
    <CardSection heading="Event List">
      <div className="btn">
        <Button primary={true} onClick={() => onAddClick()}>
          Add Sample
        </Button>
        &nbsp;&nbsp;&nbsp;
        <Button primary={true} onClick={onPrint}>
          Report
        </Button>
        &nbsp;&nbsp;&nbsp;
        <Button destructive={true} onClick={() => OnDelete()}>
          Delete Record
        </Button>
        &nbsp;&nbsp;&nbsp;
        {dialog && (
