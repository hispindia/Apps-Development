...
export const EventForm = ({ history, match }) => {
  ...
  useEffect(() => {
    if (
      eventValue?.u8VDCIwa3w4 == "Sterile" ||
      eventValue?.u8VDCIwa3w4 == "Insignificant Growth" ||
      eventValue?.u8VDCIwa3w4 == "Commensals" ||
      eventValue?.u8VDCIwa3w4 == "Growth of contaminants"
    ) {
  ...
  useEffect(() => {
    if (!isFirstRender && panelValid && eventEditable && isAddIsolate)
    ...
    <MainSection padded>
      ...
      {dialogNote ? (
        <SweetAlert
          ...
          customButtons={
            <React.Fragment>
              <Button primary={true} onClick={(e) => onClickSave(e)}>
                Save
              </Button>
              &emsp;&emsp;&emsp;
